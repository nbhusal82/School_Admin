...
const BlogCategory = () => {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <PageHeader title="Blog Categories" subtitle="Manage blog categories">
        <button onClick={() => navigate("/admin/blog")} className="p-2 hover:bg-white rounded-full border border-transparent hover:border-gray-200 transition bg-white shadow-sm mr-2">
          <ArrowLeft size={20} className="text-gray-600" />
        </button>
      ...
      <div className="max-w-2xl">
        <Table
          ...
          actions={(row) => (
            <ActionButtons
              onEdit={() => openEdit(row)}
              onDelete={() => handleDelete(row.category_id)}
            />
          )}
