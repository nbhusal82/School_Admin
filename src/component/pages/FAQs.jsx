...
const FAQPage = () => {
  ...
  const columns = [
    ...
    },
    {
      header: "Question",
      accessor: "question",
      cellClassName: "font-medium text-gray-700 max-w-xs",
    },
    {
      header: "Answer",
      accessor: "answer",
      cellClassName: "text-gray-500 max-w-sm truncate",
    },
  ];
  ...
  return (
    <div className="p-3 sm:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="lg:hidden space-y-3">
        {faqs.map((row, index) => (
          <div key={row._id || row.id} className="bg-white rounded-xl shadow-sm border p-4">
            <div className="flex justify-between items-start">
              <div className="flex-1 min-w-0 pr-2">
                <span className="text-xs text-gray-400">#{index + 1}</span>
                <p className="font-medium text-gray-700 mt-0.5">{row.question}</p>
                <p className="text-xs text-gray-500 mt-1 line-clamp-2">{row.answer}</p>
              </div>
              <ActionButtons
                onEdit={() => handleEdit(row)}
